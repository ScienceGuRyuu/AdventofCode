...
with open(batch_file_path) as passport_file:
    passports = striper(passport_file)
    tests = [Validation(passports) for passport in passports]
    part_1_count = 0
    part_2_count = 0
    for test in tests: 
        if test.check_feild_count():
            part_1_count += 1
        if test.check_passport_validity():
            part_2_count += 1
